...
class GradingStatsWorker {
 private:
  ...
  unsigned long long int secondsInYear (const std::string &timeStamp) {
    unsigned long long int yr = 0, mon = 0, day = 0, hr = 0, min = 0, sec = 0, ms = 0;
    sscanf(timeStamp.c_str(), "%4llu-%2llu-%2lluT%2llu:%2llu:%2llu.%6llu", &yr, &mon, &day, &hr, &min, &sec, &ms);
    mon *= 31;
    day += mon;
    day *= 24;
    hr += day;
  ...
  std::string whoGradedThis (const std::vector<Event> &events) {
	...
	if (lockTimes.count(e.taNetid) == 0 || lockTimes[e.taNetid] == 0) {
	  ...
	  taSeconds.push_back(std::make_pair(e.taNetid, secondsInYear(e.timeStamp) - lockTimes[e.taNetid]));
	}
      }
    }

    //taSeconds now holds the amount of time the assignment was unlocked by each TA in order

    const int MIN_SECONDS_THRESHOLD = 15;
    std::vector<std::pair<std::string, unsigned long long int>> longEnoughTaSeconds;
